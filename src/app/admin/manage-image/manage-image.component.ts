...
export class ManageImageComponent implements OnInit {
  ...
  getImage(){
    this.http.get<string[]>('http://localhost:3001/api/upload/getImage')
      .subscribe(data => {
  ...
  upload(event:any){
    ...
    this.http.post('http://localhost:3001/api/upload/single?des=category',formData).subscribe(()=>{this.getImage()})
